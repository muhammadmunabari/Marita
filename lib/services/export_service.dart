...
class ExportService {
  ...
  /// Exports a ReportModel to PDF and shares it via the native share sheet.
  static Future<void> exportReportToPdf(ReportModel report) async {
    try {
      final pdf = pw.Document();
      ...
      final cleanedContent = _cleanAIContent(report.content);
      ...
      final output = await getTemporaryDirectory();
      final file = File("${output.path}/Marita_Report_${report.reportId}.pdf");
      final bytes = await pdf.save();
      await file.writeAsBytes(bytes);
      ...
      await SharePlus.instance.share(
        ShareParams(
          files: [XFile(file.path)],
          subject: 'Marita Financial Report',
          text: 'Financial analysis report generated by Marita AI.',
        ),
      );
    } catch (e, stack) {
      if (kDebugMode) {
        debugPrint("Error exporting report to PDF: $e");
        debugPrint(stack.toString());
      }
    }
  }

  /// Uploads ReportModel PDF to Firebase Storage and shares the public URL.
  static Future<void> shareReportLink(ReportModel report, String workspaceId) async {
    try {
      if (report.shareUrl != null && report.shareUrl!.isNotEmpty) {
        await SharePlus.instance.share(
          ShareParams(
            text: 'View Marita Financial Report: ${report.shareUrl}',
            subject: 'Marita Financial Report',
          ),
        );
        return;
      ...
      await reportService.saveShareUrl(
      ...
      await SharePlus.instance.share(
        ShareParams(
          text: 'View Marita Financial Report: $url',
          subject: 'Marita Financial Report',
        ),
      );
    } catch (e, stack) {
      if (kDebugMode) {
        debugPrint("Error sharing report link: $e");
        debugPrint(stack.toString());
      }
    }
  }
